...
@Embedded
public class HbasePermissions implements Permissions {

   public static final String USER_ROLES = "users";
   public static final String GROUP_ROLES = "groups";

   @Embedded(USER_ROLES)
   private Set<HbasePermission> userPermissions = new HashSet<>();

   @Embedded(GROUP_ROLES)
   private Set<HbasePermission> groupPermissions = new HashSet<>();

   public HbasePermissions() {
   }

   public HbasePermissions(final Permissions permissions) {
      this.userPermissions = permissions.getUserPermissions().stream()
                                        .map(HbasePermission::new)
